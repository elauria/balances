...
const validateColumn = (c) => {
  if (settings.hideBalances && c.private) return false;
  return true;
};
...
const getHeader = (columns) => columns.map((c) => c.label);
...
const getFooter = async (columns, rows, balances) => {
  ...
  for (const row of [...rows, usd]) {
    row[weightIndex] =
      format((100 / total[btcValueIndex]) * row[btcValueIndex], 0.01) + "%";
  }
  return [usd, total];
};
...
const getBalances = async () => {
  ...
  for (const e of exchanges) {
    ...
    Object.keys(result.total).forEach((k) => {
      if (!result.total[k]) return;
      if (!balances[k]) balances[k] = { symbol: k.toLowerCase(), balance: 0 };
      balances[k].balance += result.total[k];
    });
  }
  ...
  try {
    ...
    for (const balance of balances) {
      ...
      if (!balance.coingecko)
        throw `Couldn't find Coingecko data for ${balance.symbol}`;
    }
    ...
    for (const balance of balances) {
      balance.coingecko.usd = usdPrices.data
        .filter((c) => c.id === balance.coingecko.id)
        .pop();
      balance.coingecko.btc = btcPrices.data
        .filter((c) => c.id === balance.coingecko.id)
        .pop();
    }
  } catch (err) {
...
const columns = [
  ...
  },
  {
    ...
    private: false,
    data: (b) =>
      b.coingecko ? b.coingecko.btc.price_change_percentage_1h_in_currency : "",
  ...
  },
  {
    ...
    data: (b) =>
      b.coingecko
        ? b.coingecko.btc.price_change_percentage_24h_in_currency
        : "",
    format: (c) => format(c, 0.01, true),
  },
  {
    ...
    private: false,
    data: (b) =>
      b.coingecko ? b.coingecko.btc.price_change_percentage_7d_in_currency : "",
    format: (c) => format(c, 0.01, true),
  },
  {
    id: "weight",
...
const main = async () => {
  try {
    if (settings.help) return printHelp();
    let header = getHeader(columns);
    let balances = await getBalances();
    await addCoingeckoData(balances.filter(validateSymbol));
    let rows = await getRows(balances.filter(validateSymbol));
    rows = rows.sort((a, b) => (a[0] < b[0] ? -1 : 1));
    if (settings.hideSmall) rows = rows.filter((r) => r[2] > 5);
